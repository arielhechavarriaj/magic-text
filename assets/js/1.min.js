...
document.addEventListener('DOMContentLoaded', function() {
    ...
    locoScroll.on('scroll', function(args) {
        let position = args.scroll.y;
        if (position > 75) {
            document.body.dataset.scrolled = 'scrolled'
        }
        if (position < 75) {
            document.body.dataset.scrolled = 'top'
        }
    });
    let showSun = !1;
    const lottiePlayer = document.querySelectorAll('.js-lottie-player');
    const sun = document.querySelector('.js-block-sun');
    const worksCounter = document.querySelector('.js-block-works-counter');
    const bannerVibesItem = document.querySelectorAll('.js-banner-vibes-item');
    locoScroll.on('call', (e) => {
        if (e === 'block-lottie-play-call') {
            lottiePlayer.forEach(player => {
                player.play()
            })
        }
        if (e === 'block-sun-player-call') {
            setTimeout(() => {
                sun.play();
                sun.dataset.show = showSun
            }, 800);
            showSun = !showSun
        }
        if (e === 'block-sun-player-bottom-call') {
            locoScroll.scroll.instance.direction === 'down' ? sun.dataset.showw = 'true' : sun.dataset.showw = 'false'
        }
        if (e === 'block-works-counter') {
            let value = parseInt(worksCounter.textContent);
            for (let i = 1; i < value + 1; i++) {
                worksCounter.textContent = '';
                worksCounter.textContent += [i]
            }
        }
        if (e === 'block-watcher') {
            let watcher = document.querySelector('.js-block-watcher.is-inview');
            let eyes = watcher.querySelectorAll('.js-block-watcher-pupils');
            let eyeLinesTop = watcher.querySelectorAll('.js-block-watcher-eye-line-top');
            let eyeLinesBottom = watcher.querySelectorAll('.js-block-watcher-eye-line-bottom');
            document.addEventListener('mousemove', (e) => {
                let x = e.clientX * 100 / window.innerWidth - 50;
                let y = e.clientY * 100 / window.innerHeight - 50;
                eyes.forEach((eye) => {
                    eye.style.transform = `translate3d(${x / 7}%, ${y / 4}%, 0)`
                });
                eyeLinesTop.forEach((eye) => {
                    eye.style.transform = `translate3d(${x / 3}%, ${y / 2}%, 0)`
                });
                eyeLinesBottom.forEach((eye) => {
                    eye.style.transform = `translate3d(${x / 3}%, ${y / 3}%, 0)`
                })
            })
        }
        if (e === 'block-shuffle') {
            bannerVibesItem.forEach(item => {
                item.classList.toggle('is-down')
            })
        }
    });
    ...
    const elTop = document.getElementById('js-top');
    // toTop.addEventListener('click', () => {
    //     locoScroll.scrollTo(elTop)
    // });
    const clocks = document.querySelectorAll('.js-clock');
    const timeDate = new Date();
    const timeUTC = timeDate.getUTCHours();
    clocks.forEach((clock) => {
        const clockShift = parseInt(clock.dataset.timezone);
        const calcShift = eval(timeUTC + clockShift);
        const hoursDegrees = ((calcShift / 12) * 360);
        clock.style.transform = `rotate(${hoursDegrees}deg)`
    })
});
...
function grad(hash, x, y) {
    let h = hash & 3;
    let u = h === 0 ? x : h === 1 ? y : x + y;
    let v = h < 2 ? y : x;
    return ((hash & 16) === 0 ? u : -u) + ((hash & 8) === 0 ? v : -v)
}

let pNoise = new Uint8Array(256);
for (let i = 0; i < 256; i++) {
    pNoise[i] = Math.floor(Math.random() * 256)
}
for (let i = 0; i < 512; i++) {
    pNoise[i] = pNoise[i & 255]
}